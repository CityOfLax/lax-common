...
namespace Lax.Data.SharePoint.Lists.Utils.Reflection {
    ...
    internal static class MemberAccessUtility {
        ...
        public static Getter CreateGetter(MemberInfo memberInfo) => CreateGetter<object, object>(memberInfo);

        public static Func<TObj, TProp> CreateGetter<TObj, TProp>(MemberInfo memberInfo) {
            var property = memberInfo as PropertyInfo;
            if (property != null) {
                return CreateGetter<TObj, TProp>(property);
            }

            var field = memberInfo as FieldInfo;
            if (field != null) {
                return CreateGetter<TObj, TProp>(field);
            }
        ...
        private static Func<TObj, TProp> CreateGetter<TObj, TProp>(FieldInfo fieldInfo) => CreateGetter<TObj, TProp>(fieldInfo.DeclaringType, fieldInfo.Name);

        #endregion

        #region [Create Setter]

        public static Action<object, object> CreateSetter(MemberInfo memberInfo) => CreateSetter<object, object>(memberInfo);
        ...
        private static Action<TObj, TProp> CreateSetter<TObj, TProp>(Type declaringType, string memberName,
            Type propertyType) {
            ...
            var propertyExpression =
                Expression.PropertyOrField(Expression.Convert(objectParameter, declaringType), memberName);

            var assignExpression = Expression.Assign(propertyExpression,
        ...
        private static Func<TObj, TProp> CreateGetter<TObj, TProp>(Type declaringType, string memberName) {
            ...
            return Expression.Lambda<Func<TObj, TProp>>(Expression.Convert(propertyExpression, typeof(TProp)),
                    objectParameter)
                .Compile();
        }
