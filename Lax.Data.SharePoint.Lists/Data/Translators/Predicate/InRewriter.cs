        ...
        protected override Expression VisitUnary(UnaryExpression node) {
            ...
            if (node.NodeType == ExpressionType.Not && innerOperand != null) {
                if (IsValidObjectInCall(innerOperand)) {
                    return RewriteNotIn((MemberExpression) innerOperand.Arguments[0],
                        (ConstantExpression) innerOperand.Arguments[1]);
                }
                if (IsValidEnumerableContainsCall(innerOperand)) {
                    return RewriteNotIn((MemberExpression) innerOperand.Arguments[1],
                        (ConstantExpression) innerOperand.Arguments[0]);
                }
                if (IsValidListContainsCall(innerOperand)) {
                    return RewriteNotIn((MemberExpression) innerOperand.Arguments[0],
                        (ConstantExpression) innerOperand.Object);
                }
            }
