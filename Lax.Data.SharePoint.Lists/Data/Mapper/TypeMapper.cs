...
namespace Lax.Data.SharePoint.Lists.Data.Mapper {
    ...
    public abstract class TypeMapper<TSPItem> {
        ...
        public void Map([NotNull] object source, [NotNull] TSPItem dest, ClientContext clientContext) {
            Guard.CheckNotNull(nameof(source), source);
            Guard.CheckNotNull(nameof(dest), dest);

            foreach (var mapper in GetMappers()) {
                mapper.Map(source, dest, clientContext);
            }
        ...
        public IReadOnlyDictionary<string, string> MapToCaml([NotNull] object source) {
            ...
            var mappers = GetMappers()
                .Where(n => n.StoreAccessor.CanSetValue ||
                            n.Field.InternalName.In(new[] {Fields.Id, Fields.BdcIdentity}));
        ...
        /// <exception cref="ArgumentNullException"><paramref name="source"/> or <paramref name="dest"/> is null.</exception>
        public void Map([NotNull] TSPItem source, [NotNull] object dest,
            IReadOnlyCollection<MemberRefModel> viewFields = null) {
            Guard.CheckNotNull(nameof(source), source);
            Guard.CheckNotNull(nameof(dest), dest);
        ...
        /// <exception cref="ArgumentNullException"><paramref name="source"/> is null.</exception>
        public IEnumerable<T> CreateAndMap<T>([NotNull] IEnumerable<TSPItem> source,
            IReadOnlyCollection<MemberRefModel> viewFields = null) {
            Guard.CheckNotNull(nameof(source), source);
        ...
        [NotNull]
        protected IEnumerable<FieldMapper<TSPItem>> GetMappers(
            [CanBeNull] IReadOnlyCollection<MemberRefModel> viewFields) {
