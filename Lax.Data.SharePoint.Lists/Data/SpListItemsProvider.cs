...
namespace Lax.Data.SharePoint.Lists.Data {
    ...
    internal partial class SpListItemsProvider : BaseSpListItemsProvider<ListItem> {
        ...
        protected override void AddInternal(IEnumerable<object> items, TypeMapper<ListItem> mapper, ClientContext clientContext) {
            foreach (var item in items) {
                var info = new ListItemCreationInformation();
                var spItem = _spList.AddItem(info);

                mapper.Map(item, spItem, clientContext);
        ...
        protected override void UpdateInternal(IEnumerable<KeyValuePair<int, object>> items, TypeMapper<ListItem> mapper) {
            ...
            foreach (var info in loadedItems) {
                mapper.Map(info.Item, info.SpItem, _clientContext);
